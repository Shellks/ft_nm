...
/*   Updated: 2025/02/27 20:00:54 by acarlott         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/ft_nm.h"

static int ft_strcmp_symbol_name(const char *s1, const char *s2)
{
    while (*s1 == '_') s1++;
    while (*s2 == '_') s2++;
...
void sort_symbols_by_name(Elf64_Sym *symbols, int symbol_count, char *strtab) {
    for (int i = 0; i < symbol_count - 1; i++) {
        for (int j = i + 1; j < symbol_count; j++) {
            if (ft_strcmp_symbol_name(&strtab[symbols[i].st_name], &strtab[symbols[j].st_name]) > 0) {
